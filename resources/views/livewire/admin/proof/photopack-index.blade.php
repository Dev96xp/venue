            ...
            <div class="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8">
                <table class="min-w-full divide-y divide-gray-300">
                    ...
                    <tbody class="divide-y divide-gray-200 bg-white">
                        ...
                        @foreach ($photopacks as $photopack)
                            <tr wire:key="photo-{{ $photopack->id }}"> {{-- SUPER IMPORTANTE --}}

                                {{-- Hace diferencia entre meses pares e impares --}}
                                {{-- MES PAR --}}
                                @if (\Carbon\Carbon::parse($photopack->scheduled_at)->month % 2 == 1)
                                    <td class="whitespace-nowrap py-1 pl-4 pr-3 text-sm sm:pl-0">
                                        <div class="flex items-center"
                                            wire:click="select_photopack({{ $photopack }})">
                                            ...
                                            <div class="ml-4">
                                                <div
                                                    class="font-bold {{ $selecionado == $photopack->name ? 'text-blue-500 text-md' : '' }}">
                                                    {{ $photopack->name }} </div>
                                                <div class="text-gray-900">{{ $photopack->addres }}</div>
                                                <div class="text-blue-500">{{ $photopack->phone }} -
                                                    {{ $photopack->city }}, {{ $photopack->zip }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    {{-- <td class="font-bold whitespace-nowrap px-3 py-1 text-sm text-gray-500 bg-blue-100">
                                        {{ \Carbon\Carbon::parse($photopack->scheduled_at)->toFormattedDateString('l j F') }}
                                    </td> --}}

                                    <td class="font-bold whitespace-nowrap px-3 py-1 text-sm text-gray-500 bg-blue-100">
                                        @isset($photopack->photoshots)
                                            @if ($photopack->photoshots->count())
                                                {{-- Name --}}
                                                <div class="ml-4">
                                                    @foreach ($photopack->photoshots as $item)
                                                        <div class="py-1">
                                                            {{ $item->status }} -
                                                            {{ \Carbon\Carbon::parse($item->scheduled_at)->toFormattedDateString('l j F') }}
